    ...
    def y(self, newY):
        ...
        if self.shape == 'square':
            if self.filled:
                ...
                self._coord = (y + w / 2.5, x + w / 2.5, y + w / 2.5 + w / 4,
                               x + w / 2.5 + w / 4)
        else:
            self._coord = (y + w / 2, x + 3 * w / 9, y + w / 2,
                           x + 3 * w / 9 + w / 4)
        ...
        if (hasattr(self, '_head')):
            if self.footprints is False:
                ...
                if self.shape == 'square':
                    ...
                    if self.filled:
                        ...
                        oldcell = (round((
                            (lll[1] - 26) / self._parentMaze._cell_width) + 1),
                                   round(((lll[0] - 26) /
                                          self._parentMaze._cell_width) + 1))
                        self._parentMaze._redrawCell(*oldcell,
                                                     self._parentMaze.theme)
                else:
                    self._parentMaze._canvas.itemconfig(
                        self._head,
                        fill=self.color.value[1])  #,outline='gray70')
                    self._parentMaze._canvas.tag_raise(self._head)
                    try:
                        self._parentMaze._canvas.tag_lower(self._head, 'ov')
                    except:
            ...
            if not self.filled or self.shape == 'arrow':
                ...
                try:
                    ...
                    pass
                self._parentMaze._redrawCell(self.x,
                                             self.y,
...
class textLabel:
    ...
    def drawLabel(self):
        self._var = StringVar()
        self.lab = Label(self._parentMaze._canvas,
...
class maze:
    ...
    def CreateMaze(self,
        ...
        def isCyclic(cell1, cell2):
            ...
            if cell1[0] == cell2[0]:
                ...
                if self.maze_map[cell1]['S'] == 1 and self.maze_map[cell2][
                        'S'] == 1:
                    if (cell1[0] + 1, cell1[1]) in self.grid and self.maze_map[
                        (cell1[0] + 1, cell1[1])]['E'] == 1:
                        ans = True
                if self.maze_map[cell1]['N'] == 1 and self.maze_map[cell2][
                        'N'] == 1:
                    if (cell1[0] - 1, cell1[1]) in self.grid and self.maze_map[
                        (cell1[0] - 1, cell1[1])]['E'] == 1:
                        ans = True
                ...
                if self.maze_map[cell1]['E'] == 1 and self.maze_map[cell2][
                        'E'] == 1:
                    if (cell1[0], cell1[1] + 1) in self.grid and self.maze_map[
                        (cell1[0], cell1[1] + 1)]['S'] == 1:
                        ans = True
                if self.maze_map[cell1]['W'] == 1 and self.maze_map[cell2][
                        'W'] == 1:
                    if (cell1[0], cell1[1] - 1) in self.grid and self.maze_map[
                        (cell1[0], cell1[1] - 1)]['S'] == 1:
                        ans = True
        ...
        def BFS(cell):
            ...
            while len(frontier) > 0:
                ...
                if self.maze_map[cell]['W'] and (cell[0],
                                                 cell[1] - 1) not in visited:
                    nextCell = (cell[0], cell[1] - 1)
                    path[nextCell] = cell
                    frontier.append(nextCell)
                    visited.add(nextCell)
                if self.maze_map[cell]['S'] and (cell[0] + 1,
                                                 cell[1]) not in visited:
                    nextCell = (cell[0] + 1, cell[1])
                    path[nextCell] = cell
                    frontier.append(nextCell)
                    visited.add(nextCell)
                if self.maze_map[cell]['E'] and (cell[0],
                                                 cell[1] + 1) not in visited:
                    nextCell = (cell[0], cell[1] + 1)
                    path[nextCell] = cell
                    frontier.append(nextCell)
                    visited.add(nextCell)
                if self.maze_map[cell]['N'] and (cell[0] - 1,
                                                 cell[1]) not in visited:
                    nextCell = (cell[0] - 1, cell[1])
                    path[nextCell] = cell
                    frontier.append(nextCell)
        ...
        if not loadMaze:
            ...
            while len(_stack) > 0:
                ...
                if (x - 1, y) not in _closed and (x - 1, y) in self.grid:
                    cell.append("N")
                if len(cell) > 0:
                    if pattern is not None and pattern.lower(
                    ) == 'h' and bias <= biasLength:
                        if ('E' in cell or 'W' in cell):
                            if 'S' in cell: cell.remove('S')
                            if 'N' in cell: cell.remove('N')
                    elif pattern is not None and pattern.lower(
                    ) == 'v' and bias <= biasLength:
    ...
    def _drawMaze(self, theme):
        ...
        self._canvas = Canvas(width=scr_width,
                              height=scr_height,
                              bg=theme.value[0])  # 0,0 is top left corner
        ...
        self._cell_width = round(
            min(((scr_height - self.rows - k * self._LabWidth) / (self.rows)),
            ...
            if self.grid is not None:
                for cell in self.grid:
                    ...
                    if self.maze_map[cell]['E'] == False:
                        l = self._canvas.create_line(y + w,
                                                     x,
                                                     y + w,
                                                     x + w,
                                                     width=2,
    ...
    def _tracePathSingle(self, a, p, kill, showMarked, delay):
        ...
        self._win.after(delay, self._tracePathSingle, a, p, kill, showMarked,
                        delay)
